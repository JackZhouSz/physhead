...
def merge_render(
    ...
    """
    Render a frozen head model composited with a trainable hair model.

    Background tensor (bg_color) must be on GPU!
    """
    means3D = torch.cat([pc.get_xyz, pc_hair.get_xyz], dim=0)
    rotations = torch.cat([pc.get_rotation, pc_hair.get_rotation], dim=0)
    scales = torch.cat([pc.get_scaling, pc_hair.get_scaling], dim=0)
    opacity = torch.cat([pc.get_opacity, pc_hair.get_opacity], dim=0)
    ...
    colors_precomp = None
    if override_color is None:
        shs = torch.cat([pc.get_features, pc_hair.get_features], dim=0)
    else:
        colors_precomp = override_color
        pc.active_sh_degree = None
