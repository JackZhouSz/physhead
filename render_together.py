...
from gaussian_renderer import merge_render
...
def render_set(dataset : ModelParams, name, iteration, views, gaussians, gaussians_hair, pipeline, background, render_mesh):
    ...
    for idx, view in enumerate(tqdm(views_loader, desc="Rendering progress")):
        if gaussians.binding != None:
            # head is frozen at a single fixed timestep during hair training —
            # keep it fixed here too, rather than animating it per-view.
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_val : bool, skip_test : bool, render_mesh: bool):
    with torch.no_grad():
        ...
        if dataset.target_path != "":
             ...
             render_set(dataset, f'{name}', scene.loaded_iter, scene.getTrainCameras(), gaussians, gaussians_hair, pipeline, background, render_mesh)
        else:
            if not skip_train:
                render_set(dataset, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, gaussians_hair, pipeline, background, render_mesh)

            if not skip_val:
                render_set(dataset, "val", scene.loaded_iter, scene.getValCameras(), gaussians, gaussians_hair, pipeline, background, render_mesh)

            if not skip_test:
                render_set(dataset, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, gaussians_hair, pipeline, background, render_mesh)
