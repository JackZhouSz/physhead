...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : Optional[BasicPointCloud], spatial_lr_scale : float, args=None):
        ...
        if self.binding is None and args is not None and args.scale_gaus:
            # hair Gaussians: re-derive scale/rotation from the Frenet-frame fit
            scale_gaus = np.load(args.scale_gaus)
            new_arr = np.ones((scale_gaus.shape[0], scale_gaus.shape[1], 3)) * 0.00001
            new_arr[:, :, 0] = scale_gaus * 0.8
            scale_gaussian = new_arr.reshape(-1, 3)
            scales = self.scaling_inverse_activation(torch.from_numpy(scale_gaussian).float().to('cuda'))

            rots = torch.from_numpy(np.load(args.rot_gaus).reshape(-1, 4)).float().to('cuda')

            opacities = inverse_sigmoid(1.0 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))

            assert fused_point_cloud.shape[0] == scale_gaussian.shape[0] == rots.shape[0], (
                "hair Gaussian count mismatch between pcd/points3d.ply and scale_gaus/rot_gaus — "
                "check for stale files in the source dataset directory"
            )
        else:
    ...
    def training_setup(self, training_args):
        ...
        if self.binding is not None and getattr(training_args, "is_hair_opt", False):
            # head is frozen while training hair color; keep a dummy no-op param
            dummy_param = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
            l = [{'params': [dummy_param], 'lr': 0.0, "name": "dummy"}]
        elif self.binding is None and getattr(training_args, "is_hair_opt", False):
            # hair Gaussians: only optimize color, geometry stays fixed from the Frenet fit
            l = [
