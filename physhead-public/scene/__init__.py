...
class CameraDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            if "Ava-256-2_EXP_cheek00" in camera.image_path:
                camera.body_mask = PILtoTorch(Image.open(camera.image_path.replace('image_corr', 'body_mask_sam')), (camera.image_width, camera.image_height))
                camera.hair_mask = PILtoTorch(Image.open(camera.image_path.replace('image_corr', 'hair_mask_sam')), (camera.image_width, camera.image_height))
...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : Union[GaussianModel, FlameGaussianModel], load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        elif os.path.exists(os.path.join(args.source_path, "canonical_flame_param.npz")) and gaussians.binding != None:
        ...
        # create gaussians
        if self.loaded_iter:
            if type(self.gaussians).__name__ == "FlameGaussianModel" and getattr(args, "head_ply", "") != '':
                self.gaussians.load_ply(args.head_ply, has_target=args.target_path != "")
            else:
