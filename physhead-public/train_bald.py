...
def training_report(tb_writer, iteration, losses, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    for key in ('xyz', 'scale', 'dynamic_offset', 'laplacian', 'dynamic_offset_std'):
        if key in losses:
            wandb_scalars[f'train_loss_patches/{key}_loss' if key in ('xyz', 'scale') else f'train_loss_patches/{key}'] = losses[key].item()
